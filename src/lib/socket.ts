...
// ---------------------------------------------------------------------------
// TURN credentials (generated by the worker via Cloudflare Calls)
// ---------------------------------------------------------------------------

let cachedTurn: { servers: RTCIceServer[]; expiresAt: number } | null = null;

/**
 * Fetch short-lived TURN servers from the worker. Cached well under the
 * credential TTL; failures return [] so the caller falls back to STUN.
 */
export async function fetchTurnServers(): Promise<RTCIceServer[]> {
  if (cachedTurn && Date.now() < cachedTurn.expiresAt) return cachedTurn.servers;
  try {
    ...
    cachedTurn = { servers, expiresAt: Date.now() + 3 * 3600_000 }; // creds live 6h
    return servers;
  } catch {
    return [];
  }
